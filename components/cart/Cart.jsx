...
const Cart = () => {
    // To access the context api function
    const  {addItemToCart,cart,deleteItemFromCart}  = useContext(CartContext)

    // To handle Quantity increase decrease buttons 
    ...
    // To Calculate The Amount and The Tax And The Total Amount]
    const amountBeforeTax = cart?.cartItems?.reduce((acc,item)=> acc + (item.quantity*item.price),0)

    const taxAmount = (amountBeforeTax*0.15).toFixed(2)

    const totalAmount = (Number(amountBeforeTax)+Number(taxAmount)).toFixed(2)
...
<Button variant="outlined" color="error" startIcon={<DeleteIcon />} onClick={()=>deleteItemFromCart(cartItem?.product)}>
  Remove
</Button>
                      </div>
                    </div>
